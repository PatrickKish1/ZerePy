...
class SonicConnection(BaseConnection):
    ...
    def get_token_info(self, token_symbol: str, chain_id: str = None, dex_id: str = None) -> list:
        """Get detailed token information by symbol, optionally filtered by chain and dex Parameters:token_symbol (str): The token symbol to search for chain_id (str, optional): Chain ID to filter results dex_id (str, optional): DEX ID to filter results Returns:list: List of dictionaries containing formatted token information
        """
        try:
            response = requests.get(
            ...
            if chain_id:
                sonic_pairs = [pair for pair in sonic_pairs if pair.get("chainId") == chain_id]
            if dex_id:
                sonic_pairs = [pair for pair in sonic_pairs if pair.get("dexId") == dex_id]

            formatted_tokens = []
            for pair in sonic_pairs:
                # Combine base and quote token information
                token_info = {
                    'chain': pair.get('chainId'),
                    'dex': pair.get('dexId'),
                    'name': f"{pair.get('baseToken', {}).get('name')} / {pair.get('quoteToken', {}).get('name')}",
            ...
            logger.error(f"Error fetching token information: {str(error)}")
            return []
    ...
    def register_actions(self) -> None:
        self.actions = {
            ...
            "get-token-info": Action(
                ...
                description="Get detailed information about token pairs by symbol"
            ),
        }
